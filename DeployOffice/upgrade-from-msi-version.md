...
- If using RemoveMSI All = True and Display Level = none to uninstall the old MSI and install Office 365 ProPlus in silent mode may fail if the old office application in open on the client machine. You can use Display Level = Full to provide a prompt to the user to close the application to proceed with  installation. To uninstall existig versions Office and install Office 365 ProPlus in silent mode (or via Configuration Manager), add following in Configuration.xml. <Property Name="FORCEAPPSHUTDOWN" Value="TRUE"/> Note that this will force the shutdown of the application without a prompt to the user.
